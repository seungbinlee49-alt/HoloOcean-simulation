...
| 닻돌/reef cue | 16기 / 4개 | **0개** (보고서 유물 카탈로그 확인 결과 전부 마도 Ⅱ지구 출수, 아래 근거 참고) |
...
- **선체 파편 1점**: 18-1구역에서 확인된 실측 저판재 추정 선체편(약 150×50×30cm, 40cm 깊이에
  매몰)을 `burial_fraction=0.8`(대부분 매몰)로 반영. 완전한 선체가 아니라 작은 파편이라는 점을
  그대로 반영했고, 정확한 grid cell 내 위치는 원문에 있지만 로컬 좌표 변환 정밀도상 그대로 옮기지
  않았습니다 (JSON `wreck_spawns[0].evidence` 참고). 테스트 트랙(y=0)과 겹치지 않도록 y=5m로 살짝
  띄워뒀습니다 — 그렇지 않으면 소나 나디르 사각지대에 항상 들어가 있어서 절대 안 찍힙니다
  (처음 캡처했을 때 실제로 이 문제를 겪고 고쳤습니다).
