...
7. **재질을 입도(grain size) 하나로만 추정하는 방식 자체가, 실측 대비 최대 5 dB 오차를 낼 수 있다는
   현장 검증 문헌이 있습니다 (2026-07-30 확인).** 손우주 외, "서해 천해환경에서 단상태 해저면
   후방산란강도 측정", 한국음향학회지 34(6), pp.444-454 (2015) — 실험 지점이 **태안반도 서쪽 해역**
   (36°53'N 126°07'E, 경기만 남부, 우리 마도 사이트와 같은 태안반도 연안)으로 지리적으로 가장 가까운
   실측 문헌입니다. 이 논문은 정확히 우리가 쓰는 것과 같은 접근(입도크기만으로 지음향 인자 추정 →
   APL-UW 산란모델 입력)을 실측 후방산란강도와 비교했는데, 수평입사각이 커질수록(>55°) 모델과 실측이
   최대 5 dB까지 벌어졌고, 입도 대신 실측 해저 거칠기 스펙트럼(다중빔음향측심기 자료)을 입력하자 오차가
   3 dB 이내로 줄었습니다. 논문 원문 결론: "기존 국내 연구는 평균 입자크기만을 이용해 산란강도를
   추정했는데, 이 방법은 해저지형 특성을 충분히 반영하지 못해 제한적이었다." 우리 프로젝트는 이 논문이
   지적하는 것과 동일한 입도-only 접근(Hamilton Table 2, 위 "해저 재질" 섹션 참고)을 쓰고 있어 같은
   한계를 그대로 안고 있습니다 — 참고로 이 논문이 실측한 이 지역 자갈모래(Mz 0.8φ)의 전밀도는 1.92
   g/cm³(1920 kg/m³)였고, 이는 우리가 KIGAM 분류로 확인한 다른 입도 등급(Very Fine Sand, Mz 3.5)과는
   달라 직접 대체값으로 쓸 수는 없습니다. 더 정밀하게 하려면 마도 현장의 실측 해저 거칠기/밀도 자료가
   필요한데, 우리에겐 그게 없어서 지금은 이 한계를 안고 Hamilton Table 2를 계속 씁니다.
